...
var pesertaController = function () {
	...
	var getPeserta = function (req, res) {
		if (req.query.ps != undefined && req.query.kode != undefined && req.query.kelas != undefined) {
			...
			getDaftarPeserta(url, ps, kode, kelas, res);
		} else {
			res.status(400);
			res.json({
...
// FUNCTION
function getDaftarPeserta(url, ps, kode, kelas, res) {
	var output = {};
			...
			if (!error && response.statusCode == 200) {
				...
				if (!available) {
					output = {
						...
						message: "Gagal mengakses halaman daftar kelas. Check kembali nilai dari program studi, kode kuliah atau kelas yang anda masukkan"
					}
					res.status(404);
					res.json(output)
				}
